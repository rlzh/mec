...
class Genius(API):
    ...
    def _result_is_lyrics(self, song_title):
        ...
        expression = r"".join(["({})|".format(term)
                               for term in default_terms]).strip('|')
        regex = re.compile(expression, re.IGNORECASE)
        return not regex.search(self._clean_str(song_title))

    def _get_item_from_search_response(self, response, type_, search_strs=None):
        ...
        for section in response['sections']:
            ...
            if hits:
                if search_strs:
                    max_count = 0
                    candidate = None
                    # iterate through hits to find best match based on search string
                    for i in range(len(hits)):
                        result = hits[i]['result']
                        full_title = result['full_title'].lower().strip()
                        # replace non-breaking line space
                        full_title = full_title.replace(u'\xa0', u' ')
                        match_count = 0
                        for search_str in search_strs:
                            search_str = search_str.lower().strip()
                            if search_str in full_title:
                                match_count += 1
                        if match_count == len(search_strs):
                            return result
    ...
    def _search_song_old(self, title, artist="",
        ...
        if self.verbose:
            if artist:
                print('Searching for "{s}" by {a}...'.format(
                    s=title, a=artist))
            else:
        ...
        results = [r['result']
                   for r in response['hits'] if r['type'] == 'song']
    ...
    def search_song(self, title, artist="", get_full_info=True):
        ...
        result = self._get_item_from_search_response(
            response, type_="song", search_strs=[title, artist])
    ...
    def search_artist(self, artist_name, max_songs=None, sort='title',
        ...
        while not reached_max_songs:
            songs_on_page = self.get_artist_songs(
                artist_id, sort, per_page, page)
            ...
            for song_info in songs_on_page['songs']:
                ...
                song_is_valid = has_title and (
                    has_lyrics or (not self.skip_non_songs))
